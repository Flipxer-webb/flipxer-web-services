...
describe("DojahService", () => {
    ...
    it("verifyBvn and verifyNin should return Dojah responses", async () => {
        dojah.verifyBvn.mockResolvedValue({
            status: true,
            data: { entity: { status: true } },
        });
        dojah.verifyNin.mockResolvedValue({
            status: true,
            data: { entity: { status: true } },
        });

        const bvn = await service.verifyBvn({
            bvn: "123",
            first_name: "A",
            last_name: "B",
            dob: "1990-01-01",
        } as any);
        const nin = await service.verifyNin({
            nin: "123",
            first_name: "A",
            last_name: "B",
            dob: "1990-01-01",
    ...
    it("verify methods should translate Dojah library errors", async () => {
        dojah.verifyNin.mockRejectedValue(
            new DojahValidationError("invalid nin"),
        );
    ...
    it("preserves Dojah library metadata on translated exceptions", async () => {
        const upstreamError = new DojahThirdPartyServiceFailureError(
        ...
        try {
            ...
            expect(typedError.responseBody).toEqual({
            ...
            expect(typedError.providerErrorName).toBe(
                "DojahThirdPartyServiceFailureError",
            );
        }
    });
    ...
    it("getVerificationResult should return normalized statuses", async () => {
        dojah.getVerificationResult
            .mockResolvedValueOnce({
                data: {
                    status: "ok",
                    entity: { status: true, verification_status: "Completed" },
                },
            })
    ...
    it("verifyBusinessDocuments should keep processing when checks reject", async () => {
        jest.spyOn(service, "lookupCAC").mockRejectedValue(
            new Error("cac down"),
        );
        jest.spyOn(service, "verifyTIN").mockRejectedValue(
            new Error("tin down"),
        );
        jest.spyOn(service, "analyzeDocument").mockRejectedValue(
            new Error("ocr down"),
        );
