...
export class QuidaxTradingProvider implements ITradingProvider {
    ...
    async getAccountDetail(
        ...
        const result = await this.quidaxService.getAccountDetail({
            user_id: userId,
        });
    ...
    async getUserWalletList(
        ...
        const result = await this.quidaxService.getUserWalletList({
            user_id: userId,
        });

        const wallets: WalletBalance[] = ((result.data as any) || []).map(
            (w: any) => ({
                currency: w.currency,
                balance: w.balance || "0",
    ...
    async getUserWallet(
        ...
        return {
            ...
            data: {
                ...
                availableBalance:
                    data?.available_balance || data?.balance || "0",
            },
        };
    }
    ...
    async getPaymentAddressList(
        ...
        const addresses: PaymentAddress[] = ((result.data as any) || []).map(
            (a: any) => ({
                id: a.id,
                address: a.address,
    ...
    async placeOrder(
        ...
        const result = await this.quidaxService.buyOrSellOrderRequest(
            options.userId,
            {
    ...
    async cancelOrder(
        ...
        const result = await this.quidaxService.cancelBuyOrSellOrderRequest(
            options.userId,
            {
                user_id: options.userId,
    ...
    async getOrderList(
        ...
        const orders: OrderResult[] = ((result.data as any) || []).map(
            (o: any) => this.mapOrderResult(o),
        );
    ...
    async createSwapQuote(
        ...
        return {
            ...
            data: {
                ...
                rate:
                    data?.rate?.toString() ||
                    data?.exchange_rate?.toString() ||
                    "0",
                fee:
                    data?.fee?.toString() || data?.total_fee?.toString() || "0",
    ...
    async createWithdrawal(
        ...
        const result = await this.quidaxService.createWithdrawerRequest({
            ...
            transaction_note:
                options.transactionNote || options.narration || "",
        });
    ...
    async getWithdrawalList(
        ...
        const withdrawals: WithdrawalResult[] = (
            (result.data as any) || []
        ).map((w: any) => this.mapWithdrawalResult(w));
    ...
    async getWithdrawalFees(
        ...
        const data: any = result.data;
        const fee =
            data?.type === "range" && Array.isArray(data?.fee)
    ...
    async fetchDeposits(
        ...
        const deposits: DepositRecord[] = ((result.data as any) || []).map(
            (d: any) => ({
                id: d.id,
                currency: d.currency,
    ...
    async getMarketTickers(): Promise<ProviderResponse<MarketTicker[]>> {
        ...
        const data: any = result.data;
        const tickers: MarketTicker[] = Object.entries(data || {}).map(
            ([pair, tickerData]: [string, any]) => ({
                pair,
                ...
                change24h:
                    tickerData?.ticker?.change || tickerData?.change || "0",
    ...
    async getSingleMarketTicker(
        ...
        const result = await this.quidaxService.getSingleMarketTicker(
            pair.toLowerCase(),
        );
