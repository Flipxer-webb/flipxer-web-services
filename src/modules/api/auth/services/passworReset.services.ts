...
export class PasswordService {
    ...
    async sendForgotPasswordEmail(userId: number, email: string, name: string, code: string) {
        try {
            // Path to the EJS email template
            const templatePath = path.join(__dirname, '../../../../../public/templates/emails/password_reset_email/html.ejs');            
            ...
            logger.error(`Error sending password reset email: ${error.message}`, { userId, email });
            logger.error(`Full error details: ${JSON.stringify(error)}`);

            // Throw custom exception if email provider fails
            throw new InvalidEmailProviderException('Failed to send password reset email. Please try again later.');
        }
    }
    ...
    async requestPasswordReset(dto: SendForgotPasswordDto) {
        try {
            ...
            if (!user) {
                logger.warn(`Password reset requested for non-existent user: ${dto.email}`);
            ...
            logger.info(`Password reset request created for user: ${user.email}`);
            return { message: 'Password reset email sent successfully' };
        } catch (error) {
            logger.error(`Error requesting password reset: ${error.message}`, { email: dto.email });
            throw new HttpException(error.message || 'Failed to request password reset', HttpStatus.BAD_REQUEST);
        }
    }
    ...
    async resetPassword(dto: ResetPasswordDto) {
        try {
            ...
            if (!user || !user.passwordResetRequest) {
                logger.warn(`Invalid password reset request for user: ${dto.email}`);
                throw new HttpException('Invalid reset request', HttpStatus.BAD_REQUEST); // Invalid reset request
            }

            if (user.passwordResetRequest.code !== dto.resetCode) {
                logger.warn(`Invalid reset code for user: ${dto.email}`);
                throw new HttpException('Invalid reset code', HttpStatus.BAD_REQUEST); // Invalid reset code
            }

            const createdAt = user.passwordResetRequest.createdAt;
            if (Date.now() - createdAt.getTime() > 30 * 60 * 1000) { // 30-minute expiration
                await this.prisma.passwordResetRequest.delete({ where: { userId: user.id } });
                logger.warn(`Expired reset code for user: ${dto.email}`);
                throw new HttpException('Reset code has expired', HttpStatus.BAD_REQUEST); // Expired reset code
            }
            ...
            logger.error(`Error resetting password: ${error.message}`, { email: dto.email });
