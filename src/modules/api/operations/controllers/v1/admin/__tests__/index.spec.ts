...
describe("Operations admin controllers", () => {
    ...
    it("delegates wallet controller endpoints", async () => {
        ...
        const controller = new AdminWalletController(walletService as any);
        ...
        await expect(controller.invalidateCache()).resolves.toEqual(expect.objectContaining({ message: "Wallet cache invalidated successfully" }));
    });
    ...
    it("delegates slack webhook controller endpoints", async () => {
        ...
        const controller = new AdminSlackWebhookController(slackService as any);
        ...
        await expect(controller.createWebhook({ name: "x" } as never)).resolves.toEqual(expect.objectContaining({ data: { id: 2 } }));
        await expect(controller.updateWebhook(3, { name: "y" } as never)).resolves.toEqual(expect.objectContaining({ data: { id: 3 } }));
        await expect(controller.deleteWebhook(4)).resolves.toEqual(expect.objectContaining({ message: "Webhook deleted successfully" }));
    ...
    it("delegates liquidity alert controller endpoints", async () => {
        ...
        const controller = new AdminLiquidityAlertController(alertService as any);
        ...
        await expect(controller.createAlert({ currency: "BTC" } as never)).resolves.toEqual({ id: 11 });
        await expect(controller.runLiquidityCheck()).resolves.toEqual({ checked: true });
        ...
        await expect(controller.escalateAlert(1)).resolves.toEqual({ escalated: true });
    });
});
