...
export class RecoveryEmailController {
    ...
    @ApiOperation({
        summary: "Send a 6-digit recovery PIN to the specified recovery email",
    })
    @ApiBody({ type: SendRecoveryPinDto })
    @ApiResponse({
        status: 201,
        description: "Recovery PIN sent successfully",
        type: Object,
    })
    ...
    @ApiResponse({
        status: 400,
        description: "Bad Request - Invalid PIN or PIN expired",
    })
    @ApiResponse({
        status: 401,
        description: "Unauthorized - Authentication required",
