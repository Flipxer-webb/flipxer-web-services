...
describe("UserService", () => {
    ...
    beforeEach(async () => {
        jest.clearAllMocks();
        mockIndividualKycStageService.ensureCurrentIndividualStageAttempts.mockResolvedValue(
            false,
        );
        ...
        mockPrismaService.deviceToken.deleteMany.mockResolvedValue({
            count: 1,
        });
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                {
                    provide: IndividualKycStageService,
                    useValue: mockIndividualKycStageService,
                },
    ...
    describe("getProfile - Business KYC Logic", () => {
        ...
        beforeEach(() => {
            ...
            mockPrismaService.assetWallet.findFirst.mockResolvedValue({
                balance: 0,
            });
        });
        ...
        it("should throw when profile user disappears before DB hydration completes", async () => {
            ...
            await expect(service.getProfile(baseUser as any)).rejects.toThrow(
                "User not found",
            );
        ...
        it("should return BUSINESS_RECORD requirement if business record is not completed", async () => {
            ...
            expect(result.data.verificationRequirements.nextStep).toBe(
                "BUSINESS_RECORD",
            );
        ...
        it("should require a current business attempt before showing WAIT_FOR_VERIFICATION", async () => {
            ...
            expect(result.data.businessVerification).toEqual(
        ...
        it("should return BUSINESS_DOCUMENT_UPLOAD when the current business attempt is rejected", async () => {
            ...
            expect(result.data.verificationRequirements.details).toBe(
                "Resubmit clearer CAC documents",
            );
            expect(result.data.businessVerification).toEqual(
                expect.objectContaining({
                    status: "REJECTED",
        ...
        it("should load profile journey inputs from current stage attempts without legacy verification rows", async () => {
            ...
            const profileQuery =
                mockPrismaService.user.findUnique.mock.calls[0][0];
        ...
        it("should return GOVERNMENT_ID with decline details for INDIVIDUAL user when BVN/NIN was rejected", async () => {
            ...
            expect(result.data).not.toHaveProperty("verificationRequirements");
            expect(result.data.kycJourney.overallStatus).toBe(
                "ACTION_REQUIRED",
            );
            expect(result.data.kycJourney.currentStage).toBe("GOVERNMENT_ID");
            expect(result.data.kycJourney.nextAction).toEqual(
        ...
        it("should prefer current KYC stage attempts when building the individual journey", async () => {
            ...
            const identityStage = result.data.kycJourney.stages.find(
                (stage: any) => stage.stage === "IDENTITY_DOCUMENT",
            );

            expect(result.data).not.toHaveProperty("verificationRequirements");
            expect(result.data.kycJourney.currentStage).toBe(
                "IDENTITY_DOCUMENT",
            );
            expect(result.data.kycJourney.nextAction.type).toBe("WAIT");
        ...
        it("should ignore conflicting legacy document, address, and income flags once current stage attempts exist", async () => {
            ...
            const addressStage = result.data.kycJourney.stages.find(
                (stage: any) => stage.stage === "ADDRESS",
            );
            const incomeStage = result.data.kycJourney.stages.find(
                (stage: any) => stage.stage === "INCOME",
            );
        ...
        it.each([
            ...
            "should derive %s journey state for INDIVIDUAL users",
            async ({
                profile,
                expectedStep,
                expectedStage,
                ...
                if (expectedStep === "COMPLETE") {
                    ...
                    expect(result.data.kycJourney.nextAction.type).toBe(
                        "COMPLETE",
                    );
                }
            },
    ...
    describe("getProfile - Stale cache guard", () => {
        ...
        it("should recompute cached profile requirements from current stage attempts", async () => {
            ...
            const result = await service.getProfile(individualUser);
            const cachedWrite =
                mockRedisCacheService.set.mock.calls.at(-1)?.[1];
            ...
            expect(cachedWrite.data).not.toHaveProperty(
                "verificationRequirements",
            );
    ...
    describe("updateNotificationToken", () => {
        ...
        it("should gracefully handle Redis invalidation failure after enabling push", async () => {
            ...
            expect(warnSpy).toHaveBeenCalledWith(
                expect.stringContaining(
                    `Failed to invalidate profile cache for user ${mockUser.id}`,
                ),
            );
        ...
        it("should disable push notifications and remove device tokens", async () => {
            ...
            expect(
                mockPrismaService.deviceToken.deleteMany,
        ...
        it("should return success (200 OK equivalent) even when Redis connection fails", async () => {
            ...
            expect(warnSpy).toHaveBeenCalledWith(
                expect.stringContaining(
                    "Failed to invalidate profile cache for user 26",
                ),
            );
            expect(warnSpy).toHaveBeenCalledWith(
        ...
        it("should handle multiple consecutive Redis failures gracefully", async () => {
            ...
            mockRedisCacheService.del.mockRejectedValue(
