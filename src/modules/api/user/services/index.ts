...
export class UserService {
  ...
  async sendRecoveryPinEmail(userId: number, email: string, name: string, pin: string) {
    try {
      ...
      throw new AuthGenericException("Failed to send recovery PIN email. Please try again later.");
    }
  }
  ...
  async sendRecoveryPin(dto: SendRecoveryPinDto): Promise<ApiResponse> {
    try {
      ...
      if (error instanceof UserNotFoundException || error instanceof AuthGenericException) {
        throw error;
      }
      logger.error(`Error in sendRecoveryPin for email ${dto.email}: ${error.message}`, { stack: error.stack });
      throw new AuthGenericException("An error occurred while sending the recovery PIN");
    }
  }
  ...
  async verifyRecoveryPin(dto: VerifyRecoveryPinDto): Promise<ApiResponse> {
    try {
      ...
      if (!user || !user.recoveryEmail) {
        ...
        throw new UserNotFoundException("User or recovery email not found");
      }
      ...
      return buildResponse({ message: "Recovery email updated successfully" });
    } catch (error) {
      if (
      ...
      throw new AuthGenericException("An error occurred while verifying the recovery PIN");
    }
  }
}