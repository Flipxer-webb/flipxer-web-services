...
export class TradingController {
    ...
    @HttpCode(HttpStatus.OK)
    @ApiOperation({ summary: 'get purchase limits for buy' })
    ...
    @HttpCode(HttpStatus.OK)
    @ApiOperation({ summary: 'initiate wallet address generation' })
    @UseGuards(AuthGuard)
    @ApiBearerAuth('access-token')
    ...
    async initiateWalletCreation(
        ...
        return await this.tradingService.initiateWalletAddressCreation(user.id, dto);
    }

    @HttpCode(HttpStatus.OK)
    @ApiOperation({
        summary: 'manual crypto account creation for user that missed auto generation',
    })
    @UseGuards(AuthGuard)
    @ApiBearerAuth('access-token')
    ...
    @HttpCode(HttpStatus.OK)
    @ApiOperation({ summary: 'confirm buy order' })
    @UseGuards(AuthGuard, TransactionAmountGuard)
    @ApiBearerAuth('access-token')
    @Post('buy/order')
    ...
    @HttpCode(HttpStatus.OK)
    @ApiOperation({ summary: 'confirm sell order' })
    @UseGuards(AuthGuard, TransactionAmountGuard)
    @ApiBearerAuth('access-token')
    @Post('sell/order')
    ...
    @ApiOperation({
        summary:
            'This endpoint initiates the withdrawal - verify the details before proceeding with the withdrawal process. Once submitted, funds cannot be recovered if sent to an incorrect address.',
    })
    @UseGuards(AuthGuard, TransactionAmountGuard)
    @ApiBearerAuth('access-token')
