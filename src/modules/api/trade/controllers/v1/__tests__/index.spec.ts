...
describe("TradingController", () => {
    ...
    beforeEach(() => {
        tradingService = {
            ...
            getSupportedPaymentMethod: jest.fn().mockResolvedValue({ data: ["bank"] }),
            getSupportedNetworks: jest.fn().mockResolvedValue({ data: ["TRC20"] }),
            getSupportedTradingPairs: jest.fn().mockResolvedValue({ data: ["BTC/USDT"] }),
            getPurchaseLimitForBuy: jest.fn().mockResolvedValue({ data: { max: 1000 } }),
            verifyWalletAddress: jest.fn().mockResolvedValue({ data: { valid: true } }),
            getCryptoWithdrawerFee: jest.fn().mockResolvedValue({ data: { fee: 2 } }),
            getWalletAddress: jest.fn().mockResolvedValue({ data: { address: "T123" } }),
            getWalletAddresses: jest.fn().mockResolvedValue({ data: [] }),
            initiateWalletAddressCreation: jest.fn().mockResolvedValue({ message: "queued" }),
            triggerQuidaxAccountCreation: jest.fn().mockResolvedValue({ message: "ok" }),
            buyCryptoQuoteRequest: jest.fn().mockResolvedValue({ data: { quote: "q1" } }),
            buyCryptoOrder: jest.fn().mockResolvedValue({ data: { orderId: 10 } }),
            getBuyOrderStatus: jest.fn().mockResolvedValue({ data: { status: "pending" } }),
            cancelBuyOrder: jest.fn().mockResolvedValue({ message: "cancelled" }),
            notifyPendingBuyOrder: jest.fn().mockResolvedValue({ message: "sent" }),
            confirmPaymentSent: jest.fn().mockResolvedValue({ message: "confirmed" }),
            sellCryptoQuoteRequest: jest.fn().mockResolvedValue({ data: { quote: "s1" } }),
            sellCryptoOrder: jest.fn().mockResolvedValue({ data: { orderId: 11 } }),
            getSwapEstimate: jest.fn().mockResolvedValue({ data: { estimate: "e1" } }),
            createInstantSwap: jest.fn().mockResolvedValue({ data: { quoteId: "swap-1" } }),
            confirmInstantSwapQuote: jest.fn().mockResolvedValue({ data: { status: "ok" } }),
            refreshInstantSwap: jest.fn().mockResolvedValue({ data: { quoteId: "swap-2" } }),
            withdrawerRequest: jest.fn().mockResolvedValue({ data: { tx: "wd-1" } }),
    ...
    it("delegates public info endpoints", async () => {
        ...
        expect(tradingService.getSupportedPaymentMethod).toHaveBeenCalledWith(paymentQuery);
        expect(tradingService.getSupportedNetworks).toHaveBeenCalledTimes(1);
        expect(tradingService.getSupportedTradingPairs).toHaveBeenCalledTimes(1);
        expect(tradingService.getPurchaseLimitForBuy).toHaveBeenCalledWith(limitQuery);
        expect(tradingService.verifyWalletAddress).toHaveBeenCalledWith(verifyDto);
        expect(tradingService.getCryptoWithdrawerFee).toHaveBeenCalledWith(feeDto);
    });
    ...
    it("delegates wallet and buy/sell flows with authenticated user", async () => {
        ...
        expect(tradingService.getWalletAddress).toHaveBeenCalledWith(77, walletDto);
        expect(tradingService.getWalletAddresses).toHaveBeenCalledWith(77, walletListDto);
        expect(tradingService.initiateWalletAddressCreation).toHaveBeenCalledWith(77, initiateWalletDto);
        expect(tradingService.triggerQuidaxAccountCreation).toHaveBeenCalledWith(user);
        expect(tradingService.buyCryptoQuoteRequest).toHaveBeenCalledWith(user, buyQuoteDto);
        expect(tradingService.buyCryptoOrder).toHaveBeenCalledWith(user, buyOrderDto);
        expect(tradingService.getBuyOrderStatus).toHaveBeenCalledWith("buy-ref", 77);
        expect(tradingService.cancelBuyOrder).toHaveBeenCalledWith("buy-ref", 77);
        expect(tradingService.notifyPendingBuyOrder).toHaveBeenCalledWith("buy-ref", 77);
        expect(tradingService.confirmPaymentSent).toHaveBeenCalledWith("buy-ref", 77);
        expect(tradingService.sellCryptoQuoteRequest).toHaveBeenCalledWith(user, sellQuoteDto);
    ...
    it("delegates swap, withdrawal and order status flows", async () => {
        ...
        const withdrawDto = { currency: "usdt", amount: "50", beneficiaryId: "ben-1" } as any;
        ...
        expect(tradingService.createInstantSwap).toHaveBeenCalledWith(user, swapDto);
        expect(tradingService.confirmInstantSwapQuote).toHaveBeenCalledWith(user, confirmSwapDto);
        expect(tradingService.refreshInstantSwap).toHaveBeenCalledWith(user, refreshSwapDto);
        expect(tradingService.withdrawerRequest).toHaveBeenCalledWith(user, withdrawDto);
